...
export class ProductControl {
  ...
  constructor  (plus: Element | null, num: HTMLInputElement | Element | null, minis: Element | null, sku: string | null) {
    if (!plus ) throw new Error('plus is null');
    if (!num) throw new Error('num is null');
    if (!(num instanceof HTMLInputElement)) throw new Error('num is not HTMLInputElement');
  ...
  public updateNum() {
    return this.getNumfromCart()
      .then((res) => this.changeNum(res))
  }
